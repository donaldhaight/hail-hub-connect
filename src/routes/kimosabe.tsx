...
export const Route = createFileRoute("/kimosabe")({
  head: () =>
    routeHead({
      title: "Kimosabe",
      description:
        "The front door to the Human Blockchain. Arrive with no email and no phone — ask once, and a file opens: a holding wallet, an append-only ledger, and a direct line to guidance.",
      path: "/kimosabe",
    }),
...
/**
 * The bare front door. A wordmark, one field, one button — nothing else until
 * the first question is asked. Asking is the gesture that opens the file.
 */
            ...
            <form onSubmit={ask} className="mt-10">
              <div className="flex border border-border transition-colors focus-within:border-navy">
                ...
                <input
                  ...
                  placeholder="Ask anything. The first question opens your file."
                  aria-label="Ask Kimosabe"
    ...
    <div className="flex min-h-screen flex-col bg-background text-foreground">
      <header className="border-b border-border">
        <div className="mx-auto flex h-14 max-w-3xl items-center justify-between px-6">
          <span className="font-serif text-lg tracking-tight text-ink">Kimosabe</span>
          <Meta truth="DECISION" confidentiality="C1" status={view ? `Anchor ${view.anchor.slice(0, 8)}` : undefined} />
        </div>
      </header>
      ...
      <main className="mx-auto w-full max-w-3xl flex-1 px-6 py-10">
        {asked ? (
          <div className="mb-8">
            ...
            <p className="mt-4 max-w-[62ch] text-sm leading-relaxed text-muted-foreground">
              A file opened the moment you asked — an opaque anchor, a holding wallet, and a
              permanent line in an append-only ledger. Kimosabe answers through guidance, not
              chatter; what follows is yours alone.
            </p>
          </div>
        ) : (
        ...
        <section className="mb-10 border border-border p-6">
          ...
          <p className="mt-3 max-w-[56ch] text-sm leading-relaxed text-muted-foreground">
            JoeBack is earned, never bought. It buys one thing: entry. Entry costs {fmt(price)}{" "}
            {PLATFORM_TOKEN}. Nothing here transfers to another person, and nothing here has
            external value until a certified role exists.
          </p>
          {view?.hasStanding ? (
        ...
        <form onSubmit={ask} className="mb-6">
          <div className="flex border border-border transition-colors focus-within:border-navy">
            ...
            <input
              ...
              value={question}
              onChange={(e) => setQuestion(e.target.value)}
              placeholder="Ask again — the file stays open."
              aria-label="Ask Kimosabe again"
